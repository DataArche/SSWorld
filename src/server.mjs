...
import { compileNamed, createProject, listProjects, readSource, writeSource, patchSource, DEFAULT_ANCHOR } from "./project.mjs";
...
Workflow: ssworld_catalog (learn available components) -> ssworld_project_create -> ssworld_source_read -> ssworld_source_write (whole file) or ssworld_source_patch (one exact span; pass the digest you read) -> ssworld_compile (real diagnostics + budget usage) -> ssworld_preview (returns a URL to open in a browser with WebGPU) -> ssworld_capture_frame (screenshot + pixel/region statistics + runtime errors mapped to scene.ssdl:line + camera pose from the open page).
...
const TOOLS = [
  ...
  },
  {
    ...
    run: ({ name, title, longitude, latitude, height, template }) => createProject(name, { title, template, anchor: {
  ...
  },
  {
    ...
    description: "Read a project's .ssdl source with its digest (needed by ssworld_source_write / ssworld_source_patch) and the list of .ssdl files in the project. file '*' returns every .ssdl file at once (sources: [{file, content, digest}]).",
    inputSchema: { type: "object", required: ["project"], properties: {
      project: { type: "string" }, file: { type: "string", description: "Relative .ssdl path; default scene.ssdl; '*' for all files." } }, additionalProperties: false },
  ...
  },
  {
    ...
    description: "Replace a whole .ssdl file. Pass expected_digest from ssworld_source_read (or 'new' for a new component file). For small edits prefer ssworld_source_patch. Does not compile; call ssworld_compile next. Editing the .ssdl files in the project directory with any other file tool is also fine: ssworld_compile always rebuilds from disk and the digest refreshes; expected_digest only guards these two tools.",
  ...
  },
  {
    ...
    run: ({ project, file, old_string, new_string, expected_digest, replace_all }) => patchSource(project, file, old_string, new_string ?? "", expected_digest, { replaceAll: Boolean(replace_all) }),
  },
  {
  ...
  },
  {
    ...
    description: "Screenshot the project's open preview page through the engine (real WebGPU frame). Returns the PNG as image content (also saved under captures/), pixel statistics (luma percentiles, under/over-exposure, green/blue/white/neutral coverage overall and per 3x3 region, top colours), runtime errors since the last hot reload mapped to the scene.ssdl line that set the member, and the camera pose (camera.source says whether it is the scene's camera or the engine default after a failed load). Requires the viewer_url from ssworld_preview to be open and visible in a browser; returns page_not_open (with page.visibility) otherwise.",
    inputSchema: { type: "object", required: ["project"], properties: {
      project: { type: "string" },
    ...
    run: async ({ project, width, height, settle_ms, timeout_ms }) => {
      ...
      const reply = await pageCommand(project, "capture", { width: width || 800, height, settle_ms: settle_ms ?? 500 }, { timeoutMs: timeout_ms || 20000 });
      if (!reply.ok) {
        const hidden = reply.page?.status?.visibility && reply.page.status.visibility !== "visible";
        const message = reply.error === "page_timeout"
          ? (hidden ? `the page is ${reply.page.status.visibility} (minimised or background tab); bring it to the front and retry` : "the page did not answer in time (is the tab visible and painting?)")
          : String(reply.error);
        throw Object.assign(new Error(message), { code: reply.error, extra: { page: reply.page } });
      }
      ...
      const camera = result.camera && !result.camera.error
        ? { ...result.camera, source: loaded ? "scene" : "engine_default",
      ...
      return {
        payload: {
          ...
          verdict: verified ? "frame captured from the running scene with no runtime errors; judge composition from the image and stats.regions"
