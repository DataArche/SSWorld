...
import { SSDL_ROOT } from "./ssdl-root.mjs";

// The asset-count wall is the source project's own (SOURCE_LIMITS.asset_refs), so it is read from there.
const { SOURCE_LIMITS } = await import(pathToFileURL(path.join(SSDL_ROOT, "compiler", "src", "source-project-0.3.mjs")).href);
...
// Labels are rasterised in the PAGE, with Canvas 2D, and handed to the engine as a bitmap on
// Label3D's image lane (LabelFacade image/image_width/image_height, 2026-09-15).  That settles the
// font question: the browser's own font stack draws the text, so CJK and emoji are glyphs with no
// font file shipped and no font licence to clear.  It also avoids Label3D's own text path, which
// is NOT the qApp->font() one people assume - with mUseSDF (the default) it goes through SdfAtlas
// and FreeType and needs a real font FILE, or it prints "font not initialized" and draws nothing.
// Measured on the real machine with NO font served: 44530 changed pixels against an 11-pixel drift
// floor (probe_label.py --no-font), which is the gate.
//
// Engine: the 2026-09-15 pull.  The engine rebuilt earlier that day drew NO label billboard at all
// (61 changed pixels, maximum channel difference 33 - noise - on either lane, and testapp showed the
// same natively, so it was never a WASM-only fault); the pulled visual/44006a6 lineage fixed it and
// the gate came back at 44472 pixels.  Two engine-side fixes ride along in this tree: label3d.cpp
// only zeroed imageWidth/imageHeight when mUrl was invalid, which made setImageWidth a no-op on the
// base64 lane, and sizes had to stop assuming the page's devicePixelRatio (capabilities() reports
// the viewer's).  Still open, and NOT label-specific: BillboardCollectionVS multiplies every
// pixel-sized billboard by czm_resolutionScale (renderWidth/displayWidth), which is 2 here, so a
// label lands at about twice its authored size.  No /2 is applied here to paper over it.
export const LABEL_POLICY = "Label draws text at a WGS84 anchor on a camera-facing billboard. The text is rasterised in the page with Canvas 2D and handed to the engine as a bitmap, so it needs no font file and CJK and emoji render as glyphs; fontSize/bold/italic/underline/fontColor/backgroundColor/strokeColor are drawn by the page and are baked into the bitmap at creation, while lineToGround (the drop line to the terrain) is drawn by the engine. fontSize is CSS pixels on screen, not metres in the world. text takes newlines for multiple lines, at most 4096 characters, at most 256 labels per scene";
...
// Polygon joined on the same terms: the native polygon now ships planar UVs (u +X, v +Y, scaled by
// its longer extent) and the constant (+X) frame, so a lake or river outline can carry water.
export const TANGENT_CAPABLE_TYPES = new Set(["Plane", "Polygon", "HeightField", "Lathe", "Tube", "Loft", "Sweep", "Torus", "Roof", "Stairs"]);
...
export const TANGENT_POLICY = "a tangent-space geometry is required: only Plane/Polygon/HeightField/Lathe/Tube/Loft/Sweep/Torus/Roof/Stairs carry analytic tangents (Mesh only when it is given uvs, ExtrudedPolygon only on its bevel/taper/axis lane, and neither is admitted by this gate), so this combination would render without tangent space and is refused at compile time";
...
// Every readable member of PointerHandler is a scalar or a boolean, on purpose: an SSDL expression
// consumes scalars and booleans, so a vector member would be readable in the catalog and unusable in
// a binding, and there is no isFinite() to test a NaN sentinel against.
export const POINTER_POLICY = "PointerHandler is the mouse over the canvas: pressed (button 0 left / 1 middle / 2 right), screenX/screenY in canvas pixels (origin top-left), hit plus hitX/hitY/hitZ for the point under the cursor in the anchor's LOCAL metres (the same frame every position member is written in - the engine's own pick answers in earth-centred metres and is converted here, so these numbers compose directly with position and api.scene.spawn), and wheelDelta for the last wheel step. hit is the guard: hitX/hitY/hitZ KEEP the last point that was hit rather than going to a sentinel, so read them only while hit is true. pressed is force-released when the pointer leaves the canvas, so a drag cannot latch on. One per scene, on the window - it is refused inside a spawnable component. Host JS gets the same pick from api.input.ray(screenX, screenY).";
...
// the asset count is the source project's SOURCE_LIMITS.asset_refs.
...
export const ASSET_LIMITS = Object.freeze({ model: 32 * 1024 * 1024, texture: 8 * 1024 * 1024, geojson: 8 * 1024 * 1024, count: SOURCE_LIMITS.asset_refs });
...
// Particles are CPU sprites. Everything an author gets wrong here is one of three things: the units
// (metres and seconds, never centimetres), the cap (maxParticles is a hard ceiling, not a hint), or
// the clock (the simulation runs whether or not anyone is looking, so screenshots need freezing).
export const PARTICLE_POLICY = `one CPU sprite emitter; particles are unlit, do not collide and cast no shadow. Lengths (position, shapeSize, size, softness, maxDrawDistance) are metres, speed m/s, gravity m/s2, lifetime seconds, spread and rotationSpeed degrees. maxParticles (create_only, default 500, max 20000) is a hard ceiling on live particles: size it from rate x lifetime[1] or the emitter silently clips, which reads as rate being ignored (project_check warns particle_budget; the whole scene is capped at ${DEFAULT_BUDGETS.particles} particles). shape cone and sphere need a non-zero shapeSize[0]; box needs a non-zero extent on some axis. blend additive for fire/glow, alpha for smoke/rain; a greyscale sprite with no alpha needs opacitySource luminance. softness is refused above 0 (particle_softness_unavailable): the soft-particle depth fade does not work on this engine build. The simulation advances off the engine clock, so set playbackSpeed 0 (plus warmup and a fixed seed) before comparing screenshots. Falling things that should not accelerate (rain, snow) take gravity 0 and a constant speed. burst / restart / clear are host-JS commands through api.scene, not SSDL members`;
...
export const CONVENTIONS = Object.freeze({
  ...
  labels: LABEL_POLICY,
  procedural_geometry: "parametric generators (HeightField width/depth/columns/rows plus heights at the (columns+1)*(rows+1) grid corners - not columns*rows - row-major from -depth/2, or noise: \"fbm\" with a seed instead of heights; Lathe profile [radius, 0, height] revolved around Z with segments and optional closed caps; Tube path + radius + segments with a parallel-transport frame; Sweep any profile drawn in XZ ([x, 0, z], x right, z up, looking along the path) swept along a path with optional twist/scaleEnd/cap, or closedProfile: false for an open strip; Loft ccw rings stacked bottom to top with optional cap, resample: true to allow different point counts and closed: true to stitch the last ring back to the first; Torus radius/tube/segments/tubeSegments; Roof a simple polygon footprint of 3..64 points with style gable/hip/shed, pitch in degrees, optional overhang and thickness - a convex quadrilateral is planned in closed form and takes ridge, any other outline is planned from its straight skeleton and takes gables (edge indices, edge i = point i to point i + 1) for gable ends and lowEdge for shed; Stairs steps/rise/run/width climbing +X from the origin, solid or floating treads; Mesh flat vertices [x, y, z, ...] and faces [a, b, c, ...] as the escape hatch for any shape a script can produce) are compile-time constants: the IR stores parameters, the runtime builds a MeshData/v1 mesh (ccw outward, at most 65535 vertices per node, compile error mesh_budget beyond that). smooth: \"catmullrom\" with samples (1..16, default 4) threads Lathe profiles, Tube/Sweep paths and Loft ring columns through a centripetal Catmull-Rom curve before the budget is counted. flat: true on a Sweep or Lathe gives every profile edge its own vertex band so the section keeps hard corners (double the vertices, same triangles); it is the opposite of smooth and the two should not be combined. ExtrudedPolygon gains bevel (top chamfer), taper (top inset) and axis (\"x\"/\"y\" for ramps and wedges); any of them moves the node onto the generated-mesh lane. Generated vertices carry normalized UVs (u around each ring or along the width, v along the profile/path/section order, both 0..1); per-vertex functions and author JavaScript are not accepted; glTF assets go through Model",
  textures: "PrincipledMaterial supports baseColorMap, metallicRoughnessMap, normalMap and emissiveMap, each referring to a Texture; texture mapping requires UVs on the target geometry. Prepare a metallicRoughnessMap in linear space: G is roughness and B is metalness, and both channels multiply the material's roughness/metalness scalar values. normalMap is tangent-space and only the Plane/Polygon/HeightField/Lathe/Tube/Loft generators carry analytic tangents, so a normal map on any other geometry is refused at compile time with material_requires_tangent; normalScale (0..2, default 1) scales its strength. Put Texture image resources at assets/*.png or assets/*.jpg, each no larger than 8 MiB. The runtime shares identical image content by content digest, so one image used by many objects or texture slots occupies one texture; reuse a path where it makes the scene easier to read. The compiler reports resolved image file bytes only and never estimates decoded memory; the runtime facade is authoritative for decoded texture accounting. uvScale is [u, v] and applies UV × uvScale; smaller values increase repeat density, while the exact rendered tiling direction still needs hardware verification. emissiveMap is sRGB colour data and, like baseColorMap, needs only UVs — never tangents — so it works on any primitive; the base pass multiplies it by emissiveColor ([r, g, b], 0..16, default 1 meaning the map as authored), and values above 1 are how a neon surface crosses the bloom threshold. emissiveColor without an emissiveMap is a flat self-lit colour",
  shading_lanes: "three material kinds, and the choice is about lighting, not about looks. PrincipledMaterial is the lit PBR lane: baseColor is relit by sun, sky and shadows, so an author-picked hex never renders as that hex. UnlitMaterial (baseColor, opacity, baseColorMap, uvScale, emissiveColor) writes its colour straight to the frame with no lighting, shadow or reflection term - the right lane for markers, legends, holograms, signage, flat-shaded blocking and anything whose colour is data rather than material. It is still tone mapped and post processed, so it is not a pixel-exact UI colour, and it carries no metalness/roughness/normalMap/normalScale/metallicRoughnessMap/emissiveMap (they are refused at compile time rather than accepted and ignored). baseColor alone cannot exceed 1, so cross the bloom threshold with emissiveColor ([r, g, b], 0..16) added on top; opacity below 1 keeps the unlit path through the translucent pass. WaterMaterial is the third lane, a prebuilt water surface rather than a general material: baseColor is the shallow colour, deepColor the colour reached after depthFadeDistance metres of water (default 150), and because that fade reads the scene depth texture there must be geometry UNDER the water surface or deepColor never appears. waveIntensity (0..4, default 0.103333, 0 = flat mirror) and flowDirection/flowSpeed drive a built-in scrolling normal map - there is no texture slot to fill and no normalMap member. It needs analytic tangents, so the host must be Plane/Polygon/HeightField/Lathe/Tube/Loft; it is always translucent and two-sided whatever opacity says; and it animates with the clock, so freeze flowSpeed at 0 before comparing screenshots. A target carries one material of any one kind",
  ...
  page: "index.html is project-owned; the template puts the WebGPU canvas in its own card with the brief and the readouts around it, so no overlay covers the canvas (an overlay over the canvas also swallows the taps TapHandlers need; give decorative overlays pointer-events: none)",
...
export function componentNotes(name) {
  ...
  if (name === "SkyAtmosphere") return { runtime_note: `the engine ships Earth defaults; the four scattering vectors and skyLuminanceFactor are refused at compile time because an evenly weighted value there turns the sky orange-brown (${SKY_TINT_POLICY}). The scalar members (multiScatteringFactor, the *Scale magnitudes, mieAnisotropy, the exponential distributions, heightFogContribution, aerial perspective) stay writable. mieAnisotropy only reshapes the MIE term, and at the default mieScatteringScale 0.003996 that term is about 8x smaller than Rayleigh, so sweeping it end to end changes nothing you can see -- measured 0 -> 0.99 at the pixel noise floor. Raise mieScatteringScale first: at 0.06, g 0 -> 0.9 moves 42% of the frame. The phase is also symmetric in g at 90 degrees from the sun, so -0.9 vs +0.9 is the one pair that shows nothing when the sun is out of shot` };
  ...
  if (name === "Label") return { runtime_note: LABEL_POLICY };
  ...
  if (name === "ParticleEmitter") return { runtime_note: PARTICLE_POLICY };
  if (name === "PointerHandler") return { runtime_note: POINTER_POLICY };
...
export function checkRuntimeSupport(sceneIR) {
  ...
  if (environments.length) {
    ...
    // Any cloudCoverage, 0 included, makes the Environment rewrite the fog density and falloff every
    // frame from its clear/cloudy curve, so the fog's own values read back as fogDensityClear (0.01)
    // and fogHeightFalloffClear (0.09) -- which is how fogDensity once got written off as a dead knob.
    if ((environments[0].properties || []).some((item) => item.property === "cloudCoverage")) {
      for (const node of (sceneIR?.nodes || []).filter((item) => item.type === "ExponentialHeightFog")) {
        for (const member of ["fogDensity", "fogHeightFalloff"]) {
          if (!(node.properties || []).some((item) => item.property === member)) continue;
          const curve = member === "fogDensity" ? "fogDensityClear / fogDensityCloudy" : "fogHeightFalloffClear / fogHeightFalloffCloudy";
          problems.push({ code: "fog_overridden", severity: "warning", node: node.id, type: node.type, property: member,
            message: `ExponentialHeightFog '${node.id}'.${member} is overwritten every frame: Environment '${environments[0].id}' sets cloudCoverage, which drives the fog from its own curve. Tune Environment.${curve} instead, or drop cloudCoverage to keep ${member} as written` });
        }
      }
    }
  }
  // The renderer draws ONE directional light, the atmosphere sun (skyatmosphererendercommon.cpp takes
  // the first enabled DirectionalLight with intensity > 0 and stops). Any other one is not drawn as a
  // light of its own, yet while its intensity is above 0 its colour replaces the sun's: a blue "moon"
  // at intensity 1 turned the daylight blue on real hardware (2026-09-25).
  const directional = (sceneIR?.nodes || []).filter((node) => node.type === "DirectionalLight");
  if (directional.length > 1) {
    const sun = directional.find((node) => (node.properties || []).some((item) => item.property === "atmosphereSunLight" && item.value === true))
      || directional[0];
    for (const node of directional.filter((item) => item !== sun)) {
      problems.push({ code: "directional_light_limit", severity: "warning", node: node.id, type: node.type,
        message: `DirectionalLight '${node.id}': the renderer draws one directional light, the sun ('${sun.id}'). '${node.id}' would not light anything itself, and while its intensity is above 0 its colour would replace the sun's. For moonlight use Environment.moonIntensity / moonColor; for fill light use SkyLight, PointLight, SpotLight or RectLight` });
    }
  }
  for (const node of sceneIR?.nodes || []) {
    const props = new Map((node.properties || []).map((item) => [item.property, item.value]));
