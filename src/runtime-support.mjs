...
export const CONVENTIONS = Object.freeze({
  ...
  labels: LABEL_POLICY,
  procedural_geometry: "parametric generators (HeightField width/depth/columns/rows plus heights at the (columns+1)*(rows+1) grid corners - not columns*rows - row-major from -depth/2, or noise: \"fbm\" with a seed instead of heights; Lathe profile [radius, 0, height] revolved around Z with segments and optional closed caps; Tube path + radius + segments with a parallel-transport frame; Sweep any profile drawn in XZ ([x, 0, z], x right, z up, looking along the path) swept along a path with optional twist/scaleEnd/cap, or closedProfile: false for an open strip; Loft ccw rings stacked bottom to top with optional cap, resample: true to allow different point counts and closed: true to stitch the last ring back to the first; Torus radius/tube/segments/tubeSegments; Roof a convex 4-point footprint with style gable/hip/shed, pitch in degrees, optional overhang and thickness; Stairs steps/rise/run/width climbing +X from the origin, solid or floating treads; Mesh flat vertices [x, y, z, ...] and faces [a, b, c, ...] as the escape hatch for any shape a script can produce) are compile-time constants: the IR stores parameters, the runtime builds a MeshData/v1 mesh (ccw outward, at most 65535 vertices per node, compile error mesh_budget beyond that). smooth: \"catmullrom\" with samples (1..16, default 4) threads Lathe profiles, Tube/Sweep paths and Loft ring columns through a centripetal Catmull-Rom curve before the budget is counted. ExtrudedPolygon gains bevel (top chamfer), taper (top inset) and axis (\"x\"/\"y\" for ramps and wedges); any of them moves the node onto the generated-mesh lane. Generated vertices carry normalized UVs (u around each ring or along the width, v along the profile/path/section order, both 0..1); per-vertex functions and author JavaScript are not accepted; glTF assets go through Model",
